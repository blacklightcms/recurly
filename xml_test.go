...
func TestXML_NullString(t *testing.T) {
	t.Run("ZeroValue", func(t *testing.T) {
		var s recurly.NullString
		...
		if s.String() != "" {
			t.Fatalf("unexpected value: %s", s.String())
		} else if ptr := s.StringPtr(); ptr != nil {
			t.Fatalf("expected nil: %#v", ptr)
		}
	})

	s := recurly.NewString("t")
	if value, ok := s.Value(); !ok {
		t.Fatal("expected ok to be true")
	} else if value != "t" {
		t.Fatalf("unexpected value: %s", value)
	...
	type testStruct struct {
		XMLName xml.Name           `xml:"test"`
		Value   recurly.NullString `xml:"s"`
	}

	t.Run("Encode", func(t *testing.T) {
		for i, tt := range []struct {
			value  recurly.NullString
			expect string
		}{
			{value: recurly.NewString("z"), expect: `<test><s>z</s></test>`},
			{value: recurly.NewString(""), expect: `<test><s></s></test>`},
			{expect: `<test></test>`}, // zero value
		} {
			if xml, err := xml.Marshal(testStruct{Value: tt.value}); err != nil {
				t.Fatalf("%d %#v", i, err)
	...
	t.Run("Decode", func(t *testing.T) {
		for i, tt := range []struct {
			expect recurly.NullString
			input  string
		}{
			{expect: recurly.NewString("z"), input: `<test><i>z</i></test>`},
			{expect: recurly.NewString(""), input: `<test><i></i></test>`},
			{input: `<test></test>`}, // zero value
		} {
			var dst testStruct
			if err := xml.Unmarshal([]byte(tt.input), &dst); err != nil {
...
func TestXML_NullStringPtr(t *testing.T) {
	i := recurly.NewStringPtr(nil)
	if value, ok := i.Value(); ok {
		t.Fatal("expected ok to be false")
	} else if value != 0 {
		t.Fatalf("unexpected value: %s", value)
