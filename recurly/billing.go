...
func (service BillingService) Get(accountCode string) (*Response, Billing, error) {
	...
	var b Billing
	res, err := service.client.do(req, &b)
...
func (service BillingService) CreateWithToken(accountCode string, token string) (*Response, Billing, error) {
	action := fmt.Sprintf("accounts/%s/billing_info", accountCode)
	req, err := service.client.newRequest("POST", action, nil, Billing{Token: token})
	if err != nil {
		return nil, Billing{}, err
	}
...
func (service BillingService) CreateWithCC(accountCode string, params Params) {
	// @todo
}
...
func (service BillingService) CreateWithBankAccount(accountCode string, params Params) {
	// @todo
}
...
func (service BillingService) UpdateWithToken(accountCode string, token string) (*Response, Billing, error) {
	action := fmt.Sprintf("accounts/%s/billing_info", accountCode)
	req, err := service.client.newRequest("PUT", action, nil, Billing{Token: token})
	if err != nil {
		return nil, Billing{}, err
	}
...
// method instead.
// https://docs.recurly.com/api/billing-info#create-billing-info-credit-card
func (service BillingService) UpdateWithCC(accountCode string, params Params) {
	// @todo
}
...
// Account details. You will need to have the ACH feature on your site for this call to work.
// https://docs.recurly.com/api/billing-info#create-billing-info-bank-account
func (service BillingService) UpdateWithBankAccount(accountCode string, params Params) {
	// @todo
}
...
func (service BillingService) Clear(accountCode string) (*Response, error) {
	action := fmt.Sprintf("accounts/%s/billing_info", accountCode)
