...
type (
	...
	Client struct {
		...
		Accounts      recurly.AccountsService
		Adjustments   recurly.AdjustmentsService
		Billing       recurly.BillingService
		Coupons       recurly.CouponsService
		Redemptions   recurly.RedemptionsService
		Invoices      recurly.InvoicesService
		Plans         recurly.PlansService
		AddOns        recurly.AddOnsService
		Subscriptions recurly.SubscriptionsService
		Transactions  recurly.TransactionsService
	}
)
...
func (c *Client) newRequest(method string, action string, params recurly.Params, body interface{}) (*http.Request, error) {
...
func (c *Client) do(req *http.Request, v interface{}) (*recurly.Response, error) {
	req.Close = true
	...
	response := &recurly.Response{Response: resp}
	decoder := xml.NewDecoder(resp.Body)
	if response.IsError() { // Parse validation errors
		if response.StatusCode == 422 {
			var ve struct {
				XMLName          xml.Name                 `xml:"errors"`
				Errors           []recurly.Error          `xml:"error"`
				Transaction      recurly.Transaction      `xml:"transaction,omitempty"`
				TransactionError recurly.TransactionError `xml:"transaction_error,omitempty"`
			}
